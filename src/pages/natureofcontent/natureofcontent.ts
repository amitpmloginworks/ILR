...
/**
 * Generated class for the NatureofcontentPage page.
 *
...
@IonicPage() 
@Component({
  selector: 'page-natureofcontent',
  templateUrl: 'natureofcontent.html',
})
export class NatureofcontentPage {
  UsrTxtType
  NatureConSelect
  ...
  contents
   ...
   console.log("ContentNature ==",this.bdata.ContentNature)
  ...
  .flatMap(()=>this.security.contentNatureListHome())       
  .subscribe(data=>{
    loading.dismiss()
    console.log(data);
    let obj=data.content_list; 
      let field='name';     
  ...
  console.log('usertexttype',this.UsrTxtType)
  }) 

   //this.UsrTxtType=this.bdata.ContentNature
...
let backAction = platform.registerBackButtonAction(() => {
 ...
 console.log(''+event.value[i].id)
    ...
    Observable.of(loading).flatMap(loading=>loading.present())
          .flatMap(() => this.security.getNatureofContent(getContentId))
         .subscribe(data=>{ 
           loading.dismiss()
  ...
  onChangeTxtType(){
    console.log('natureconselect',this.NatureConSelect)
    ...
    Observable.of(loading).flatMap(loading=>loading.present())
          .flatMap(() => this.security.getNatureofContent(this.NatureConSelect))
         .subscribe(data=>{ 
           loading.dismiss()
           console.log("Nature===",data)
           if(data.status==0) {
...
{
  console.log(this.LikeCountTotal[i].like_id);
...
console.log("lkih==",this.LikeCountTotal[i].like_id);
  }
  }
  }
...
this.views(this.contentid)
  ...
  fav(id)
  {
    ...
    .subscribe(data=>{
      loading.dismiss()
      this.navCtrl.setRoot(NatureofcontentPage)
        console.log(data);
...
Observable.of(loading).flatMap(loading=>loading.present())
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad NatureofcontentPage');
  }
